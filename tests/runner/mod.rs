...
impl Runner {
	pub(crate) fn new(prefix: &str) -> CargoResult<Self> {
		if !RUSTC_ENV_SET.swap(true, atomic::Ordering::SeqCst) {
			let toolchain = env::var("CARGO_UDEPS_TEST_TOOLCHAIN")
				.unwrap_or_else(|_| DEFAULT_TOOLCHAIN.to_owned());
			...
			if !status.success() {
				return Err(failure::err_msg(format!("{}", status))
					.context(failure::err_msg(format!(
						"could not get the {} rustc",
						toolchain
					)))
					.into());
			}
	...
	pub(crate) fn run(self) -> CargoResult<(i32, String)> {
		...
		let mut config =
			cargo::Config::new(stderr, self.cwd.path().to_owned(), self.homedir.clone());
